...
class JsonAssetLoader(val handle: FileHandle) {
    ...
    private fun configTexturePacks(assets: Assets, json: JsonValue) {
        val root = json.get(JsonAssetLoader.Companion.TEXTURE_PACK_LABEL)
    ...
    private fun configShaders(assets: Assets, json: JsonValue) {
        val root = json.get(JsonAssetLoader.Companion.SHADER_LABEL)

        for (shader in root.JsonIterator()) {
            assert(shader.has(JsonAssetLoader.Companion.SHADER_VERT_LABEL))
            assert(shader.get(JsonAssetLoader.Companion.SHADER_VERT_LABEL).isString)
            assert(shader.has(JsonAssetLoader.Companion.SHADER_FRAG_LABEL))
            assert(shader.get(JsonAssetLoader.Companion.SHADER_FRAG_LABEL).isString)

            val vert = withRoot(shader.get(JsonAssetLoader.Companion.SHADER_VERT_LABEL).asString())
            val frag = withRoot(shader.get(JsonAssetLoader.Companion.SHADER_FRAG_LABEL).asString())
    ...
    private fun configAnimations(assets: Assets, json: JsonValue) {
        val root = json.get(JsonAssetLoader.Companion.ANIMATION_LABEL)
    ...
    private fun configMusic(assets: Assets, json: JsonValue) {
        val root = json.get(JsonAssetLoader.Companion.MUSIC_LABLE)
